...
1. 将 R 进制数的每一位数值用R^k^形式表示，即幂的底数是 R，指数为 k，k
与该位和小数点之间的距离有关；
...
1. 二进制 10100.01 = 1 X 2^4^ + 1 X 2^2^ + 1 X 2 ^-2^ = 20.25
2. 七进制 604.01 = 6 X 7^2^ + 4 X 7^0^ + 1 X 7 ^-2^ = 298.02
...
3. 补码：正数的补码与原码相同，负数的补码是其**反码末位加 1** （符号位不娈）
4. 移码：补码的符号位按位取反
